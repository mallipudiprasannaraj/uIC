...
@app.route('/brand/<int:brand_id>/',methods=['POST','GET'])
def showModels(brand_id):
    brand = session.query(Brand).filter_by(id=brand_id).one()
    if request.method=='POST':
        return "receive data from form"
    else:
        return render_template('showModels4User.html',brand=brand)
    # try:
    #     # session = DBSession()
    #     brand = session.query(Brand).filter_by(id=brand_id).one()
    ...
    #             ModelName.name).all()
    #     print "hi i am in side try"
    #     if not model:
    #         '''flash message'''
    #         flash("No Models to show.")
    #     # session.close()
    #     return render_template('showModels4User.html')
    #     # return render_template(
    #     #     'showModels4User.html',
    #     #     brand=brand,
    #     #     model=model,
    #     #     cur_user=login_session['username'])
    # except Exception as E:
    #     return render_template('showModels4User.html')
